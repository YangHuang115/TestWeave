...
class TestTaskService:
    ...
    def generate_next_task_no(db: Session, project_id: str) -> str:
        ...
        stmt = select(TestTask.task_no).where(
    ...
    def get_task_by_id(db: Session, project_id: str, task_id: str) -> TestTask:
        ...
        task = db.scalar(stmt)
        if not task:
            raise AppError(code="TEST_TASK_NOT_FOUND", message="测试任务不存在或无权限访问", status_code=404)
        return task
    ...
    def _validate_owner(db: Session, project_id: str, owner_id: str) -> None:
        ...
        if not db.scalar(member_stmt):
            raise AppError(code="TEST_TASK_OWNER_INVALID", message="负责人不是该项目的有效成员", status_code=400)

    @staticmethod
    def _validate_times(db: Session, planned_start_at: datetime, planned_end_at: datetime, version: Version) -> None:
        """校验任务时间是否合规"""
        start_naive = planned_start_at.replace(tzinfo=None) if planned_start_at.tzinfo else planned_start_at
        end_naive = planned_end_at.replace(tzinfo=None) if planned_end_at.tzinfo else planned_end_at

        if end_naive <= start_naive:
            raise AppError(code="TEST_TASK_DATE_INVALID", message="任务计划结束时间不能早于计划开始时间", status_code=400)
        
        # 校验版本上限
        if version.planned_end_at:
            version_end_naive = version.planned_end_at.replace(tzinfo=None) if version.planned_end_at.tzinfo else version.planned_end_at
            if end_naive > version_end_naive:
                raise AppError(code="TEST_TASK_END_AFTER_VERSION_END", message="任务计划结束时间不能晚于版本截止时间", status_code=400)
    ...
    def create_task(
        ...
        if version.status == "ARCHIVED":
            raise AppError(code="TEST_TASK_VERSION_ARCHIVED", message="所属版本已归档，无法创建任务", status_code=400)
        ...
        if task_type == "CASE_DESIGN":
            if not requirement_id:
                raise AppError(code="TEST_TASK_REQUIREMENT_REQUIRED", message="用例设计任务必须关联一个有效需求", status_code=400)
            ...
            if not req:
                raise AppError(code="TEST_TASK_REQUIREMENT_INVALID", message="关联的需求无效或不属于当前项目", status_code=400)
    ...
    def update_requirements(
        ...
        # 清除旧绑定，绑定新关联
        db.query(TestTaskRequirement).filter(
    ...
    def update_participants(
        ...
        if user_uuids:
            ...
            if len(valid_members) != len(user_uuids):
                raise AppError(code="TEST_TASK_PARTICIPANT_INVALID", message="部分参与人不是该项目的有效成员", status_code=400)

        # 清除旧绑定并重建
        db.query(TestTaskParticipant).filter(
    ...
    def transition_status(
        ...
        if from_status == "ARCHIVED" and target_status != "previous_status":
            raise AppError(code="TEST_TASK_ARCHIVED_READ_ONLY", message="已归档任务为只读状态，无法直接修改状态", status_code=400)

        # READY 前置校验
        if target_status == "READY":
            if not task.title:
                raise AppError(code="TEST_TASK_TITLE_REQUIRED", message="任务标题不能为空", status_code=400)
            
            # 校验版本是否已归档
            version = db.get(Version, task.version_id)
            if version and version.status == "ARCHIVED":
                raise AppError(code="TEST_TASK_VERSION_ARCHIVED", message="所属版本已归档，无法流转任务状态", status_code=400)
            ...
            if task.task_type == "CASE_DESIGN":
                # 校验必须且只能关联一个需求
                link_count = db.query(TestTaskRequirement).filter(
                    TestTaskRequirement.task_id == task.id
                ).count()
                if link_count != 1:
                    raise AppError(
                ...
                raise AppError(
                    code="TEST_EXECUTION_MODULE_NOT_AVAILABLE",
                    message="用例执行模块尚未接入，执行任务当前无法进入待开始状态",
                    status_code=400,
                )

        # READY -> IN_PROGRESS
        if from_status == "READY" and target_status == "IN_PROGRESS":
        ...
        if target_status == "BLOCKED":
            if from_status != "IN_PROGRESS":
                raise AppError(code="TEST_TASK_INVALID_TRANSITION", message=f"不能从 {from_status} 状态进入阻塞", status_code=400)
            if not reason_code or not reason_text or not reason_text.strip():
                raise AppError(code="TEST_TASK_BLOCK_REASON_REQUIRED", message="阻塞状态下必须填写阻塞原因及说明", status_code=400)
        ...
        if from_status == "BLOCKED" and target_status == "IN_PROGRESS":
            if not reason_text or not reason_text.strip():
                raise AppError(code="TEST_TASK_UNBLOCK_NOTE_REQUIRED", message="解除阻塞必须填写解决说明", status_code=400)
        ...
        if target_status == "COMPLETED":
            if from_status != "IN_PROGRESS":
                raise AppError(code="TEST_TASK_INVALID_TRANSITION", message=f"任务不能从 {from_status} 状态直接完成", status_code=400)
            ...
            if db.scalar(active_blockage_stmt):
                raise AppError(code="TEST_TASK_HAS_ACTIVE_BLOCKAGE", message="任务处于阻塞状态，无法完成", status_code=400)
        ...
        if from_status == "COMPLETED" and target_status == "IN_PROGRESS":
            if not is_admin_or_lead:
                raise AppError(code="FORBIDDEN", message="只有测试负责人或项目管理员可重新打开已完成任务", status_code=403)
            if not reason_text or not reason_text.strip():
                raise AppError(code="TEST_TASK_REOPEN_REASON_REQUIRED", message="重新打开已完成的任务必须填写重新打开原因", status_code=400)
        ...
        if target_status == "CANCELLED":
            if from_status in ["COMPLETED", "ARCHIVED"]:
                raise AppError(code="TEST_TASK_INVALID_TRANSITION", message=f"任务已完成或归档，不能取消", status_code=400)
            if not reason_text or not reason_text.strip():
                raise AppError(code="TEST_TASK_CANCEL_REASON_REQUIRED", message="取消任务必须填写原因", status_code=400)
        ...
        if from_status == "CANCELLED" and target_status == "DRAFT":
            if not reason_text or not reason_text.strip():
                raise AppError(code="TEST_TASK_RESTORE_REASON_REQUIRED", message="恢复取消的任务必须填写原因", status_code=400)
            
            # 校验版本是否已归档
            version = db.get(Version, task.version_id)
            if version and version.status == "ARCHIVED":
                raise AppError(code="TEST_TASK_VERSION_ARCHIVED", message="所属版本已归档，无法恢复取消的任务", status_code=400)

        # COMPLETED/CANCELLED -> ARCHIVED (归档)
        if target_status == "ARCHIVED":
            if from_status not in ["COMPLETED", "CANCELLED"]:
                raise AppError(code="TEST_TASK_INVALID_TRANSITION", message="只有已完成或已取消的任务才可以进行归档", status_code=400)
            if not is_admin_or_lead:
                raise AppError(code="FORBIDDEN", message="只有项目管理员或测试负责人有权限归档任务", status_code=403)
        ...
        if from_status == "ARCHIVED" and target_status == "previous_status":
            if not is_admin_or_lead:
                raise AppError(code="FORBIDDEN", message="只有管理员有权限恢复已归档任务", status_code=403)
            if not reason_text or not reason_text.strip():
                raise AppError(code="TEST_TASK_RESTORE_REASON_REQUIRED", message="恢复归档的任务必须填写原因", status_code=400)
        ...
        # 检查目标状态合法性
        valid_statuses = ["DRAFT", "READY", "IN_PROGRESS", "BLOCKED", "COMPLETED", "CANCELLED", "ARCHIVED"]
        if target_status not in valid_statuses:
            raise AppError(code="TEST_TASK_INVALID_TRANSITION", message=f"非法的目标任务状态: {target_status}", status_code=400)
