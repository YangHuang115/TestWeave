...
class RepositorySyncManager:
    @staticmethod
    def create_sync_job(db: Session, project_id: str, repository_id: str, actor_id: str | None = None) -> RepositorySyncJob:
    ...
    def poll_and_execute_jobs(cls, db: Session, worker_id: str = "worker-default") -> int:
        ...
        stmt = select(RepositorySyncJob).where(
        ...
        # 加锁租约 (lease 10 mins)
        lease_until = datetime.now(UTC) + datetime.timedelta(minutes=10) if hasattr(datetime, "timedelta") else datetime.now(UTC)
        # 用 Python 的 timedelta
        import datetime as dt_mod
        lease_until = datetime.now(UTC) + dt_mod.timedelta(minutes=10)
        ...
        try:
            ...
            logger.exception("仓库同步失败", job_id=job.id, repository_id=job.repository_id)
            db.rollback()
            # 标记失败
            ...
            if job:
                job.status = "FAILED"
                job.completed_at = datetime.now(UTC)
                job.error_code = getattr(e, "code", "SYNC_ERROR")
                job.error_message = str(e)
                # 退避 30s 重新可用
                job.available_at = datetime.now(UTC) + dt_mod.timedelta(seconds=30)
    ...
    def sync_repository(cls, db: Session, repository_id: str, job_id: str) -> None:
        ...
        if not repo or not repo.enabled:
            raise AppError(code="REPOSITORY_DISABLED", message="代码仓库已被禁用或不存在", status_code=400)

        # 解密凭证
        cred = CryptoService.decrypt(repo.credential_ref) if repo.credential_ref else None

        # 确定稳定的 data 目录绝对路径（本文件在 testweave/modules/repositories/sync.py，向上推五级）
        ...
        # 判断是否首次克隆
        git_target_url = repo.remote_url
        if repo.auth_type == "HTTP_TOKEN" and cred:
            parsed = urlparse(repo.remote_url)
            if parsed.scheme in ("http", "https"):
                netloc_with_token = f"x-token-auth:{cred.strip()}@{parsed.netloc}"
                git_target_url = parsed._replace(netloc=netloc_with_token).geturl()

        if not os.path.exists(local_dir):
            # 首次 Mirror 克隆
            args = ["git", "clone", "--mirror", git_target_url, local_dir]
            GitClient.run_git_command(args, auth_type=repo.auth_type, credential_content=cred)
        else:
            # 增量 Fetch
            args = ["git", "--git-dir=" + local_dir, "fetch", "--all", "--prune"]
            GitClient.run_git_command(args, auth_type=repo.auth_type, credential_content=cred)
        ...
        if repo.last_synced_head_sha:
            try:
                ...
                check_args = ["git", "--git-dir=" + local_dir, "cat-file", "-e", repo.last_synced_head_sha]
                GitClient.run_git_command(check_args, auth_type=repo.auth_type, credential_content=cred)

                args_log = [
                    "git", "--git-dir=" + local_dir, "log", 
                    f"{repo.last_synced_head_sha}..{repo.main_branch}",
                    f"--pretty=format:{log_format}"
                ]
                res = GitClient.run_git_command(args_log, auth_type=repo.auth_type, credential_content=cred)
        ...
        for c in reversed(commits_data):  # 从最老提交向最新提交写入，保证拓扑或时间自增
            ...
            if not db_commit:
                ...
                cls._process_commit_files(db, local_dir, db_commit.id, sha, repo.id, patch_base, repo.auth_type, cred)

        # 3. 校验并重建主干可达性
        # 如果是 force_push 或者是增量扫描，我们直接遍历库中该 repo 所有 is_reachable_from_main 的提交，
        # 并运行 git merge-base --is-ancestor 重新校准
        reach_stmt = select(GitCommit).where(
            GitCommit.repository_id == repo.id,
            GitCommit.is_reachable_from_main == True
        )
        active_commits = db.scalars(reach_stmt).all()
        for ac in active_commits:
            # 运行 ancestor 校验
            chk_args = [
                "git", "--git-dir=" + local_dir, "merge-base",
                "--is-ancestor", ac.sha, repo.main_branch
            ]
            try:
                GitClient.run_git_command(chk_args, auth_type=repo.auth_type, credential_content=cred)
            except Exception:
                # 退出码不为 0 代表已不可达
        ...
        if newly_synced_shas:
            MatcherService.match_commits_to_requirements(db, str(repo.project_id), str(repo.id), newly_synced_shas)

    @classmethod
    def _parse_git_log_output(cls, output: str) -> list[dict]:
        results = []
        if not output.strip():
            return results
    ...
    def _process_commit_files(
        ...
        args_files = [
            "git", "--git-dir=" + local_dir, "diff-tree",
            "--no-commit-id", "--name-status", "-r", "-m", sha
        ]
        try:
            res = GitClient.run_git_command(args_files, auth_type=auth_type, credential_content=cred)
        except Exception:
            return
        ...
        args_num = [
            "git", "--git-dir=" + local_dir, "show",
            "--format=", "--numstat", sha
        ]
        try:
            res_num = GitClient.run_git_command(args_num, auth_type=auth_type, credential_content=cred)
        ...
        for path, info in files_map.items():
            ...
            if not is_binary and change_type != "DELETE":
                ...
                try:
                    res_p = GitClient.run_git_command(args_patch, auth_type=auth_type, credential_content=cred)
        ...
        # 更新 commit 汇总指标
        stmt_update_commit = update(GitCommit).where(
