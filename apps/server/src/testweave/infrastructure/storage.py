...
class LocalStorageProvider(StorageProvider):
    ...
    def _get_filepath(self, storage_key: str) -> str:
        ...
        if normalized_key.startswith("..") or os.path.isabs(normalized_key):
            raise AppError(
                code="PATH_TRAVERSAL_DETECTED", message="非法的文件存储路径", status_code=400
            )
    ...
    def validate(filepath: str) -> None:
        ...
        with open(filepath, "rb") as f:
            ...
            if header != b"PK\x03\x04":
                raise AppError(
                    ...
                    message="文件不是合规的 ZIP/DOCX 压缩包格式",
                    status_code=400,
                )
        ...
        try:
            with zipfile.ZipFile(filepath, "r") as zf:
                ...
                if len(infolist) > 100:
                    raise AppError(
                        code="FILE_SAFETY_VIOLATION",
                        message="压缩包内文件条目过多，可能存在安全风险",
                        status_code=400,
                    )
                ...
                if total_compressed_size > 0:
                    ...
                    if ratio > 100.0:
                        raise AppError(
                            code="FILE_SAFETY_VIOLATION",
                            message="文件解压比率异常过高，可能为压缩炸弹",
                            status_code=400,
                        )
        except zipfile.BadZipFile:
            raise AppError(
                code="INVALID_FILE_TYPE",
                message="文件内容已损坏或不是有效的 ZIP/DOCX 压缩格式",
                status_code=400,
            ) from None
